...
class Flow:
    ...
    def is_doh(self) -> bool:
        return (
            (
                self.dest_ip in constants.DOH_LOCAL
                and self.dest_port in constants.DOH_LOCAL_PORT
            )
            or self.src_ip in constants.DOH_IPS
            or self.dest_ip in constants.DOH_IPS
        )
